...
func (fa *FlowAction) Run(context context.Context, inputs map[string]*data.Attribute, handler action.ResultHandler) error {
	...
	op := instance.OpStart
	/***** wenyan change, to compile ****/
	retID := false //comment this line
	...
	if exists {
		...
		if ok {
			op = ro.Op
			/***** wenyan change, to compile ****/
			retID = ro.ReturnID //comment this line
	...
	go func() {
		...
		defer handler.Done()
		/****** Wenyan change, this portion overrides reply data, how to set ExplicitReply??? */
